...
const confirmSelection = async () => {
  ...
  if (props.params?.slot_index !== undefined) {
    ...
    console.log(`Updating slot ${props.params.slot_index} with:`, player);
    ...
    if (saved) {
      ...
      config.players[props.params.slot_index] = {
        id: player.id,
  ...
  if (props.params?.from) {
    emit('navigate', props.params.from, { match_id: props.params.match_id });
  } else {
    emit('back');
  }
};
...
<template>
  <div class="fixed inset-0 bg-white text-slate-900 flex flex-col z-[60]">
    ...
    <header class="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-white sticky top-0 z-10">
      <button @click="props.params?.from ? emit('navigate', props.params.from, { match_id: props.params.match_id }) : emit('back')" class="p-2 -ml-2 hover:bg-slate-50 rounded-full transition-colors">
    ...
    <div class="p-4 bg-white">
      <div class="flex items-center gap-3 bg-slate-100 rounded-full px-4 h-10">
        <Search class="w-4 h-4 text-slate-400" />
        <input type="text" :placeholder="T.search_placeholder" class="flex-1 bg-transparent text-sm outline-none" />
        <button @click="onScanCode" class="p-1 hover:bg-slate-200 rounded-full transition-colors">
          <Scan class="w-5 h-5 text-slate-500" />
        </button>
      </div>
    </div>
    ...
    <div class="px-4 py-3 border-b border-slate-100 bg-slate-50">
      <div class="flex items-center gap-2">
        <div class="flex-1 bg-white rounded-xl border border-slate-200 px-3 h-11 flex items-center shadow-sm focus-within:border-red-500 transition-colors">
          <UserPlus class="w-4 h-4 text-slate-400 mr-2" />
        ...
        <button @click="quickAddPlayer" 
                class="h-11 px-5 bg-red-500 text-white rounded-xl text-sm font-bold active:scale-95 transition-all shadow-lg shadow-red-500/20">
          添加
        </button>
      </div>
    </div>
    ...
    <!-- List Title -->
    <div class="flex items-center justify-between px-4 py-2 bg-slate-50 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
    ...
    <div class="flex-1 overflow-auto pb-32">
      <div v-for="(player, idx) in players" :key="player.id" 
        ...
        <div class="relative w-10 h-10">
          <img :src="player.avatar" class="w-full h-full rounded-full bg-slate-100 object-cover" />
        ...
        <div class="flex-1">
          ...
          <div class="text-[10px] text-slate-400 font-medium">{{ T.handicap }}: {{ player.handicap }}</div>
        </div>
        <div class="w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center"
          ...
          <Check v-if="player.selected" class="w-3 h-3 text-white stroke-[4]" />
        </div>
      </div>
    ...
    <div v-if="showDialog" class="fixed inset-0 z-[70] flex items-center justify-center p-6">
      ...
      <div class="relative w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
        ...
        <div class="space-y-4">
          <div>
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">{{ T.name }}</label>
            <input v-model="tempPlayer.nickname" type="text" :placeholder="T.input_name" 
                   class="w-full h-11 bg-slate-50 rounded-xl px-4 text-sm outline-none focus:ring-2 focus:ring-red-500/20 transition-all" />
          </div>

          <div>
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">{{ T.gender }}</label>
          ...
          <div>
            <div class="flex justify-between items-center mb-1.5">
              <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ T.initial_handicap }}</label>
