  ...
  actions: {
    ...
    calculateRuleProfitWithCarryover(holeIndex: number, rule: PKRule, carryover: number): { profits: number[], nextCarryover: number } {
      ...
      if (rule.type === 'vegas_big') {
        ...
        if (isLandlord8421) {
          // 两两比 8421 加总得净分差 pairSum = Σ(L−Pi)；结算与打3分相同按「每农民单位」：
          // 每位农民 −pairSum×基数×倍率，地主 +(pairSum×农民人数)×基数×倍率
          // 例：L=2,P=[4,1] → pairSum=-1 → 农民各 +1，地主 −2
        ...
        if (winA === 0) {
          ...
          const addCollectedBase = !isMultiplierTieHole(th);
